...
/**
 * Avro serialization: 786 K req/s
 * Avro serialization: 330 K req/s
 */
final public class FreelanceClientAvro implements IFreelanceClient {
...
//        ConcurrentLinkedQueue<Object[]> reply_queue;
        ...
        final DatumWriter<MessageAvro> writer = new SpecificDatumWriter<>(MessageAvro.getClassSchema());
        final SpecificDatumReader<MessageAvro> reader = new SpecificDatumReader<>(MessageAvro.getClassSchema());
        final MessageAvro avroReply = new MessageAvro();

        public static final FlowAvro FLOW = new FlowAvro("68718", "orderId", "Sales", "EMEA", "Region");
        public static final UserAvro USER = new UserAvro("system-msg-user@BA-FR", "email", "BA-FR", "");
        public static final EntityAvro ENTITY = new EntityAvro("98658097", "invoiceId", "Invoice");
        public static final OperationAvro OPERATION = new OperationAvro();
...
//            reply_queue = new ConcurrentLinkedQueue<Object[]>();
            ...
            OPERATION.setName("368a");
            OPERATION.setService("omsf");
            OPERATION.setEnvironment("development");
        }
        ...
        public void send_request(Request request, Instant now) throws Exception {
            out.reset();
            OPERATION.setTimestamp(now.toEpochMilli());
            OPERATION.setDurationUs(5013630);
            writer.write(new MessageAvro(
                    request.msg_id,
                    request.msg,
                    "Command",
                    now.toEpochMilli(),
            ...
            this.backend_socket.send(out.toByteArray(), ZMQ.DONTWAIT);
        ...
        public int on_reply_message(Instant now) throws Exception {
            ...
            if (data.length == 0) {
                ...
                reader.read(avroReply, decoder);
                final Request request = this.requests.get(avroReply.getId());
                if (request != null) {
                    this.send_reply(now, request.msg_id, avroReply.getName().toString(), server_hostname);
